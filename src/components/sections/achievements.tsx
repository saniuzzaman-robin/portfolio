...
function AchievementCard({ item }: { item: AchievementItem }) {
  ...
  const c = ACCENT_CLASSES[item.color];
  ...
  return (
    <div
      ref={ref}
      className={`relative glass rounded-sm border ${c.borderMedium} ${c.glow} p-7 transition-all duration-500 group animate-scale-in overflow-hidden flex flex-col gap-4`}
    ...
    >
      ...
      <div className="flex items-center justify-between relative z-10">
        ...
        <span
          className={`text-[10px] uppercase tracking-widest font-space-grotesk font-bold px-2 py-0.5 rounded-full ${c.tag}`}
        >
          {item.tag}
        </span>
      </div>

      <div className={`font-space-grotesk font-bold tabular-nums text-5xl relative z-10 ${c.neon}`}>
        {displayStat}
      </div>
