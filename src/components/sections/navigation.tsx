        ...
        <div className="hidden items-center gap-1 md:flex">
          {navItems.map((item) => {
            if (item.kind === 'link') {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`font-space-grotesk group relative flex items-center gap-1.5 rounded-sm px-4 py-2 text-xs tracking-widest uppercase transition-all duration-300 lg:text-sm ${
            ...
            return (
              ...
              >
                <button
                  className={`font-space-grotesk relative flex cursor-pointer items-center gap-1.5 rounded-sm px-4 py-2 text-xs tracking-widest uppercase transition-all duration-300 lg:text-sm ${
                    groupActive || isOpen
                      ? 'text-primary-50'
                      ...
                      {item.children.map((child, idx) => {
                        ...
                        return (
                          <Link
                            key={child.href}
                            href={child.href}
                            className={`font-space-grotesk group animate-dropdown-item relative flex items-center gap-3 px-4 py-3 text-xs tracking-widest uppercase transition-all duration-200 lg:text-sm ${
                              active
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5">
          <div className="flex flex-col gap-0.5">
            ...
            <p className="font-space-grotesk text-xs tracking-wider text-neutral-50 lg:text-sm">
              Software Engineer
            </p>
          </div>
        ...
        <div ref={drawerLinksRef} className="flex flex-1 flex-col gap-1 overflow-y-auto px-4">
          ...
          <div className="overflow-hidden rounded-sm border border-white/5">
            <button
              ...
              className={`font-space-grotesk group relative flex w-full items-center gap-4 px-4 py-3.5 text-xs tracking-widest uppercase transition-all duration-200 lg:text-sm ${
                ...
                {groupItem.children.map((child) => {
                  ...
                  return (
                    <Link
                      ...
                      href={child.href}
                      onClick={() => setIsMenuOpen(false)}
                      className={`font-space-grotesk group relative flex items-center gap-4 py-3 pr-4 pl-10 text-xs tracking-widest uppercase transition-all duration-200 lg:text-sm ${
                        active
                          ? 'text-primary-50 bg-primary-50/8'
        ...
        <div className="px-6 pt-4 pb-8">
          ...
          <p className="text-neutral-40 font-space-grotesk mb-1 text-xs tracking-widest uppercase lg:text-sm">
            Available for work
          </p>
          <div className="flex items-center gap-2">
            <span className="bg-primary-50 h-1.5 w-1.5 animate-pulse rounded-full" />
            <span className="text-primary-50 font-space-grotesk text-xs lg:text-sm">
